...
const createAndSendMessage = async (waClient) => {
  ...
  payments.forEach((payment) => {
    ...
    payment.items.forEach((item, idx) => {
      message += `   ${idx + 1}. ${item.name}: Rp${item.price.toLocaleString(
        "id-ID"
      )}\n`;
    });
    ...
    if (payment.payments.length !== 0) {
      ...
      payment.payments.forEach((pay, idx) => {
        message += `   ${idx + 1}. ${pay.date.getDate()}/${
          pay.date.getMonth() + 1
        }/${pay.date.getFullYear()}: Rp${pay.amount.toLocaleString("id-ID")}\n`;
      });
    }

    message += `💥️ *Sisa: Rp${payment.remains.toLocaleString(
      "id-ID"
    )}* 💥️\n\n`;
